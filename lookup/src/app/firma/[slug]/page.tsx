...
export default async function CompanyProfilePage({ params }: { params: Promise<{ slug: string }> }) {
	...
	const averageRating =
		reviewCount > 0 ? (company.reviews.reduce((acc, r) => acc + r.rating, 0) / reviewCount).toFixed(1) : undefined
						...
						<div className='flex-shrink-0'>
							{company.logo ? (
								<div className='w-32 h-32 relative rounded-2xl overflow-hidden border border-gray-100 shadow-sm'>
									<Image src={company.logo} alt={`Logo ${company.name}`} fill className='object-cover' />
								</div>
							) : (
