...
import { approveClaim, rejectClaim, deleteCompany } from '@/actions/adminActions'
		...
		<div className='space-y-10'>
			...
			<section>
				<div className='flex items-center gap-3 mb-6'>
					...
					<div>
						<h2 className='text-xl font-bold text-gray-900'>Oczekujące decyzji</h2>
						<p className='text-sm text-gray-500'>Weryfikuj właścicieli i zarabiaj.</p>
					</div>
				</div>
							...
							>
								...
								<div className='relative z-10 flex-1 space-y-4'>
									...
									<div className='flex items-center gap-3'>
										...
										<div>
											<h3 className='font-bold text-lg text-gray-900 leading-tight'>{claim.company?.name ?? '—'}</h3>
											<p className='text-xs text-gray-500'>NIP: {claim.company?.nip || 'Brak'}</p>
										</div>
									</div>
									...
									<div className='bg-gray-50 p-4 rounded-xl border border-gray-100 space-y-2'>
										<div className='flex items-center gap-2 text-sm text-gray-700 font-medium'>
											<User size={16} className='text-gray-400' /> {claim.fullName || '—'}
										</div>
								...
								<div className='relative z-10 flex flex-row sm:flex-col justify-center gap-3 border-t sm:border-t-0 sm:border-l border-gray-100 pt-4 sm:pt-0 sm:pl-6'>
									<form action={approveClaim.bind(null, claim.id)} className='w-full'>
									...
									<form action={rejectClaim.bind(null, claim.id)} className='w-full'>
			...
			{/* SEKCJA 2: HISTORIA */}
			<section>
				<h3 className='text-gray-400 text-xs font-bold uppercase tracking-wider mb-4 ml-1'>Historia Decyzji</h3>
						...
						<tbody className='divide-y divide-gray-100'>
							{historyClaims.map(claim => (
								<tr key={claim.id} className='hover:bg-gray-50'>
									<td className='px-6 py-4 font-medium text-gray-900'>{claim.company?.name ?? '—'}</td>
