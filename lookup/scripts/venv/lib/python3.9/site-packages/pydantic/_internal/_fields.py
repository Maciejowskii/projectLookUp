...
def collect_model_fields(  # noqa: C901
    ...
    *,
    ...
    bases = cls.__bases__
    parent_fields_katalogo: dict[str, FieldInfo] = {}
    for base in reversed(bases):
        if model_fields := getattr(base, '__pydantic_fields__', None):
            parent_fields_katalogo.update(model_fields)
    ...
    for ann_name, (ann_type, evaluated) in type_hints.items():
        ...
        if assigned_value is PydanticUndefined:  # no assignment, just a plain annotation
            if ann_name in annotations or ann_name not in parent_fields_katalogo:
                # field is either:
                ...
                field_info = parent_fields_katalogo[ann_name]._copy()
