/**
 * Skrypt do generowania automatycznych opinii dla firm
 * 
 * Użycie:
 * npx tsx scripts/generate-reviews.ts
 * 
 * Generuje opinie dla 80% firm (losowo 1-10 opinii per firma)
 * Używa OpenAI do generowania naturalnych opinii
 */
...
async function main() {
  ...
  if (!openaiApiKey) {
    console.error('❌ Błąd: Brak OPENAI_API_KEY w zmiennych środowiskowych')
    process.exit(1)
  }
  ...
  console.log(`   - Przetworzono firm: ${companiesProcessed}`)
  console.log(`   - Utworzono opinii: ${totalReviewsCreated}`)
  console.log(`   - Średnia opinii per firma: ${(totalReviewsCreated / companiesProcessed).toFixed(1)}`)
}
